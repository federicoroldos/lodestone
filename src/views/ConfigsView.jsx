...
export function ConfigsView() {
  ...
  async function save() {
    try {
      ...
      toast.success('Saved (.bak created). Restart to apply.');
    } catch (e) { toast.error(e.message); }
  }
    ...
    <Card>
      ...
      <CardContent>
        <p className="text-xs text-muted-foreground mb-3">
          A timestamped <code className="rounded bg-muted px-1 py-0.5 text-xs font-mono">.bak</code> is saved before overwriting. Requires a restart.
        </p>
        <textarea
          value={content}
          onChange={e => setContent(e.target.value)}
          spellCheck={false}
          className="w-full h-[52vh] rounded-md border border-input bg-console-bg px-3 py-2 font-mono text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring/50 resize-y"
        />
